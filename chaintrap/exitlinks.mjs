...
function be16ToNumber(bytes, start=0) {
    if (bytes.length - start < 2) throw Error(`conversion from bytes to 16 bit number requires 2 bytes. Got start=${start} and length ${bytes.length}`);
...
export class Exit {
    static fromHex(linkloc) {
        var b = bytes(linkloc);
        if (b.length !== 4) throw new Error(`linkloc must be 4 hex bytes, not ${linkloc.length}`);
...
export class Link {
    ...
    native() {
        return [this.kind, this.exits, this.key, this.autoclose, this._locked, this._open];
    }
}
